...
from src.utils.core import format_time, generate_batches
...
class SequentialModel:
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            y_hat = np.zeros_like(y_train).get()
            for idx, (x_batch, y_batch) in \
                    tqdm(enumerate(generate_batches(x_train, y_train, bs))):
            ...
            y_hat = np.zeros_like(y_test).get()
            for idx, (x_batch, y_batch) in \
                    enumerate(generate_batches(x_test, y_test, bs)):
                x_batch = np.asarray(x_batch)
                y_batch = np.asarray(y_batch)

                y_hat_batch = self._forward(x_batch, training=False)
                y_hat_batch = y_hat_batch.get()
                n_start = idx * bs
                n_end = n_start + y_hat_batch.shape[0]
                y_hat[n_start:n_end, :] = y_hat_batch
    ...
    def predict(self, x: np.array) -> np.array:
        ...
        """
        x = np.asarray(x)
        return self._forward(x, training=False).get()
